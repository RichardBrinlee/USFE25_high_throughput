...
# access all the keys in the JSON file
for key in data:
    if len(data[key]) == 3:
        lattices = []
        lattices = [data[key][0]]
        box_type = data[key][1]
        set_type = data[key][2]
        file_python_alloy = key

        # variables that need to reset for each loop
        lattice_set_num = 134
        ...
        # print the key file
        print(file_python_alloy)

        # Example of using file_new instead of file_inner
        file_python_use = f'{file_python_alloy}'
        shutil.rmtree(file_python_use, ignore_errors=True)
        os.makedirs(file_python_use)
        changing = 0.99
        lattice_nums = -1  # Adjust this value as needed
  
        # place these files in the directory for the alloy 
        source = f'{file_python_original}/crawler_batch.sh'
        shutil.copy(source, file_python_use)
        source = f'{file_python_original}/crawler_gsfe.sh'
        shutil.copy(source, file_python_use)

        # Loop through the lattices and create the files
        while changing > 0:
            p = 1
            ...
            while p <= 20:
                ...
                with open(file_inner + '/lmp_gsfe.in', 'w') as f:
                    # Ensure lattice_nums is within the valid range
                    if lattice_nums < -len(lattices) or lattice_nums >= len(lattices):
                        raise IndexError("lattice_nums is out of range")
                    full_message = f"{message_1}{lattice_message_1}{lattices[lattice_nums][0]}{lattice_message_2}{message_2}{box_type}{set_type}{changing} {lattice_set_num}{message_3}"
            ...
            # make the changing variable go down by 0.01 exactly each time
